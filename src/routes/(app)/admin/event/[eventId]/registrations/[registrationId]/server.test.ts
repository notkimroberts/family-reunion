...
/* params carries BOTH ids now, and they have to agree with the registration the mock returns — the
   action 404s a registration that does not belong to the event in the URL. That pairing is pinned
   directly further down. */
function makeEvent(params: { eventId?: string; registrationId?: string } = {}) {
    return {
        request: new Request('http://localhost/admin/event/evt-1/registrations/reg-1', {
            method: 'POST',
        }),
        url: new URL('http://localhost/admin/event/evt-1/registrations/reg-1'),
        params: {
            eventId: params.eventId ?? 'evt-1',
            registrationId: params.registrationId ?? 'reg-1',
        },
...
describe('POST /admin/event/[eventId]/registrations/[registrationId] save', () => {
...
describe('POST /admin/event/[eventId]/registrations/[registrationId] reissue_link', () => {
...
/* The URL names an event AND a registration, and before the move nothing made them agree: the lookup
   was global. /admin/event/<2024>/registrations/<a-2026-registration> would have rendered and EDITED the
   2026 party inside the 2024 shell. */
describe('the event in the URL must own the registration', () => {
    it.each([
        ['save', () => actions.save(makeEvent({ eventId: 'a-different-event' }))],
        ['reissue_link', () => actions.reissue_link(makeEvent({ eventId: 'a-different-event' }))],
    ])('404s %s when the registration belongs to another event', async (_label, run) => {
        await expect(run()).rejects.toMatchObject({ status: 404 })
    })

    it('writes nothing when the pairing fails', async () => {
        await expect(
            actions.save(makeEvent({ eventId: 'a-different-event' })),
    ...
    it('does not re-issue a link for a registration in another event', async () => {
        await expect(
            actions.reissue_link(makeEvent({ eventId: 'a-different-event' })),
        ).rejects.toMatchObject({ status: 404 })

        expect(mockReissueManagementLink).not.toHaveBeenCalled()
    })
})
