...
let filteredAttendees = $derived(
    adminCtx.selectedEventId === 'all'
        ? data.attendees
        : data.attendees.filter((a) => a.eventId === adminCtx.selectedEventId),
)
...
<section class="col-span-12">
    <div class="flex items-center gap-3">
        <h1>Attendees</h1>
        <Badge variant="secondary">{filteredAttendees.length}</Badge>
    </div>
...
{#if filteredAttendees.length === 0}
    <section class="col-span-12">
        <p class="text-muted-foreground">No paid attendees for the selected year.</p>
    </section>
{:else}
    <section class="col-span-12">
        <AdminDataView>
            {#snippet mobileCards()}
                <div class="space-y-3">
                    {#each filteredAttendees as a (a.id)}
                            ...
                            <div class="flex items-start justify-between gap-2">
                                <div class="min-w-0">
                                    ...
                                    <p class="text-xs text-muted-foreground mt-0.5">
                                        {a.eventYear} · {a.tierLabel}{#if born}
                                            · b. {born}
                                        {/if}
                                    </p>
