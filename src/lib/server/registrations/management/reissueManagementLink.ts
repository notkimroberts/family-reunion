...
/* Emails the registrant a fresh management link, for when an organiser needs to get them back
   ...
   sha256(token), so the original plaintext cannot be recovered by anyone, including an admin. The
   outgoing hash is demoted rather than discarded, so the previously-sent link keeps working for the
   grace period — see isManagementTokenValid. Only links two generations old stop working.
...
export async function reissueManagementLink(params: {
    ...
    /* Only now that delivery is confirmed. Demotes the outgoing hash rather than discarding it, so
       the link they were sent before this one keeps working for the grace period. */
