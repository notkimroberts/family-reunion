...
- `admin/+layout.server.ts` returns `events` (four columns only), `currentEventId` for the pages that have no id in their URL, and `isOwner` so the registrations page can hide the Setup entry. Hiding is not the protection; the server guards every Setup route.
- **There is no admin header.** Admin renders inside the ordinary app shell — the same `AppHeader` as every other page, which already carries the theme toggle and the account menu. An `AdminHeader` existed briefly and duplicated all three of those on every admin screen. What is genuinely admin-specific lives on the page: the reunion title, the year picker and the Setup link are in the status card on the registrations list, beside the numbers they apply to.
...
- **Getting back from Setup**: `/admin/setup` has a "← Registrations" link and each Setup page has a breadcrumb to `/admin/setup`. With no header, those are the only route back — do not remove them.
