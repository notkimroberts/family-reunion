...
- Bucket variables: `BUCKET_NAME`, `BUCKET_ENDPOINT`, `BUCKET_ACCESS_KEY_ID`, `BUCKET_SECRET_ACCESS_KEY`, optional `BUCKET_REGION`. The client throws a named error when they are missing rather than degrading — a half-configured bucket that accepts uploads and loses them is worse than one that refuses.
